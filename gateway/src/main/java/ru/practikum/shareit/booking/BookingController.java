...
public class BookingController {
    ...
    public ResponseEntity<Object> updateBooking(@RequestHeader("X-Sharer-User-Id") long userId,
        ...
        if (approved == null) throw new IllegalArgumentException("approved is null");
    ...
    public ResponseEntity<Object> getBookingsByOwner(@RequestHeader("X-Sharer-User-Id") long userId,
             ...
             @PositiveOrZero @RequestParam(name = "from", defaultValue = "0") Integer from,
             @Positive @RequestParam(name = "size", defaultValue = "10") Integer size) {
        BookingStateForFind state = BookingStateForFind.from(stateParam)
                .orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
        log.info("GET booking with state={}, item owner={}, from={}, size={}", stateParam, userId, from, size);
